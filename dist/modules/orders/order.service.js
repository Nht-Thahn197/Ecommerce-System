...
exports.updatePaymentStatus = exports.updateOrderItemStatus = exports.recalculateOrderStatus = exports.listOrderItemsForSeller = exports.getOrderForUser = exports.listOrdersForUser = void 0;
...
const listOrdersForUser = async (userId, query) => {
...
const listOrderItemsForSeller = async (userId, query) => {
    const shops = await prisma_1.default.shops.findMany({
    ...
    const [total, items] = await Promise.all([
        prisma_1.default.order_items.count({ where }),
        prisma_1.default.order_items.findMany({
            where,
            orderBy: { created_at: "desc" },
            skip,
            take: limit,
...
exports.listOrderItemsForSeller = listOrderItemsForSeller;
const canCustomerCancel = (status) => status === "pending" || status === "confirmed";
const canSellerCancel = (status) => status === "pending" || status === "confirmed";
const isValidTransition = (current, next, actor) => {
    if (current === next)
        return false;
    if (actor === "seller") {
        if (next === "confirmed")
        ...
        if (next === "cancelled")
            return canSellerCancel(current);
        return false;
    }
    if (next === "received")
        return current === "delivered";
    if (next === "cancelled")
        return canCustomerCancel(current);
    return false;
};
const recalculateOrderStatus = async (orderId, tx) => {
    ...
    const allTerminal = statuses.every((status) => TERMINAL_STATUSES.includes(status));
...
exports.recalculateOrderStatus = recalculateOrderStatus;
const updateOrderItemStatus = async (userId, itemId, nextStatus) => {
    ...
    if (!isValidTransition(item.status, nextStatus, actor)) {
    ...
    await prisma_1.default.$transaction(async (tx) => {
        ...
        if (item.order_id) {
            await (0, exports.recalculateOrderStatus)(item.order_id, tx);
        }
    });
