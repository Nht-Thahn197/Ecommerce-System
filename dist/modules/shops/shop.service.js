...
const normalizeShippingConfig = (config) => ({
...
const normalizeTaxInfo = (taxInfo) => ({
...
const normalizeIdentityInfo = (identityInfo) => ({
...
const buildOnboardingData = (input) => {
...
const saveShopDetails = async (tx, shopId, input, documents) => {
    const pickupAddress = normalizeAddress(input.pickup_address || input.address);
    const taxAddress = normalizeAddress(input.tax_address);
...
const registerShop = async (userId, input) => {
    if (!input.name?.trim()) {
        throw new Error("Shop name is required");
    }
    const existing = await prisma_1.default.shops.findFirst({
    ...
    const contactPhone = sanitizeString(input.contact_phone) ||
        normalizeAddress(input.pickup_address || input.address)?.contact_phone;
    const shop = await prisma_1.default.$transaction(async (tx) => {
...
const getMyShops = async (userId, query) => {
    const { page, limit, skip } = getPagination(query);
    const where = { owner_id: userId };
    const [total, shops] = await Promise.all([
        prisma_1.default.shops.count({ where }),
...
exports.listApprovedShops = listApprovedShops;
const listPendingShops = async (query) => {
    const { page, limit, skip } = getPagination(query);
...
exports.listPendingShops = listPendingShops;
const getShopDetailById = async (shopId) => {
    const shop = await prisma_1.default.shops.findUnique({
        where: { id: shopId },
        select: {
            id: true,
            name: true,
...
const updateShopStatus = async (adminId, shopId, input) => {
    const shop = await prisma_1.default.shops.findUnique({
        where: { id: shopId },
        select: { id: true, status: true },
    });
    ...
    if (input.status === "approved") {
        return prisma_1.default.shops.update({
            where: { id: shopId },
            data: {
                status: "approved",
                approved_by: adminId,
